...
check_feature_data <- function(
    feature_data,
    check_limits = TRUE, mz_limits = c(10, 2000), rt_limits = c(0, 20), log_messages = FALSE) {
...
read_from_excel <- function(file, sheet = 1, id_column = NULL, corner_row = NULL, corner_column = NULL,
  ...
  if (!skip_checks) {
    pheno_data <- check_pheno_data(
      x = pheno_data, id_prefix = id_prefix,
...
find_mz_rt_cols <- function(feature_data) {
  ...
  if (is.null(mz_col)) {
    stop(paste0(
      "No mass to charge ratio column found - should match one of:\n",
      paste(mz_tags, collapse = ", "), " (not case-sensitive)"
    ))
  }
  if (is.null(rt_col)) {
    stop(paste0(
      "No retention time column found - should match one of:\n",
